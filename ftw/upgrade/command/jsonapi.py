...
class APIRequestor:
    ...
    def GET(self, action, site=None, instance_name=None, **kwargs):
        return self._make_request(
            "GET", action, site=site, instance_name=instance_name, **kwargs
        )

    def POST(self, action, site=None, instance_name=None, **kwargs):
        return self._make_request(
            "POST", action, site=site, instance_name=instance_name, **kwargs
        )
...
def add_requestor_authentication_argument(argparse_command):
    argparse_command.add_argument(
        "--auth", help='Authentication information: "<username>:<password>"'
    )
...
def add_site_path_argument(argparse_command):
    ...
    group.add_argument("--site", "-s", help="Path to the Plone site.")
    group.add_argument(
        "--pick-site",
        "-S",
        action="store_true",
    ...
    def func_wrapper(*args, **kwargs):
        try:
            ...
            try:
                ...
                mimetype = cgi.parse_header(response.headers.get("Content-Type"))[0]
...
def _get_running_instance(buildout_path, instance_name=None):
    for zconf in find_instance_zconfs(buildout_path, instance_name):
        ...
        if is_port_open(port):
            return {"port": port, "path": zconf.dirname().dirname()}
    return None
