...
COMMAND_LAYER = ConsoleScriptLayer(
    "ftw.upgrade", bases=(BUILDER_LAYER,), name="ftw.upgrade:command"
)
...
class UpgradeLayer(PloneSandboxLayer):
    ...
    def setUpZope(self, app, configurationContext):
        ...
        xmlconfig.file("configure.zcml", ftw.upgrade, context=configurationContext)
        ...
        try:
            ...
            xmlconfig.file(
                "configure.zcml", collective.indexing, context=configurationContext
            )
    ...
    def fix_plone_app_jquery_version(self, portal):
        ...
        version = max(
            map(itemgetter("dest"), portal_setup.listUpgrades(profileid, show_old=True))
        )
...
UPGRADE_FUNCTIONAL_TESTING = FunctionalTesting(
    bases=(UPGRADE_LAYER, set_builder_session_factory(functional_session_factory)),
