...
class UpgradeInformationGatherer:
    ...
    def get_profiles(self, proposed_only=False, propose_deferrable=True):
        profiles = self._sort_profiles_by_dependencies(
            self._get_profiles(
                proposed_only=proposed_only, propose_deferrable=propose_deferrable
            )
        )
    ...
    get_upgrades = deprecated(get_profiles, "get_upgrades was renamed to get_profiles")
    ...
    def get_upgrades_by_api_ids(self, *api_ids, **kwargs):
        ...
        missing_api_ids = set(api_ids) - set(map(itemgetter("api_id"), upgrades))
        if missing_api_ids:
    ...
    def _get_profile_data(
        self, profileid, proposed_only=False, propose_deferrable=True
    ):
    ...
    def _get_profile_upgrades(
        ...
        all_upgrades = list(
            flatten_upgrades(self.portal_setup.listUpgrades(profileid, show_old=True))
        )
    ...
    def _is_deferrable(self, upgrade_step_info):
        ...
        upgrade_step_class = getattr(maybe_ftw_upgrade_step_wrapper, "handler", None)
