...
class PloneSiteAPI(APIView):
    ...
    def execute_upgrades(
        self, upgrades, allow_outdated=False, intermediate_commit=False
    ):
    ...
    @action("POST", rename_params={"profiles": "profiles:list"})
    def execute_profiles(self, profiles, force_reinstall=False, allow_outdated=False):
        """Executes a list of profiles, each identified by their ID."""
        if not allow_outdated:
            self._require_up_to_date_plone_site()
        self._validate_profile_ids(*profiles)
        return self._install_profiles(*profiles, force_reinstall=force_reinstall)
    ...
    def _refine_profile_info(self, profile):
        return {
            ...
            "upgrades": list(map(self._refine_upgrade_info, profile["upgrades"])),
        }
    ...
    def _refine_upgrade_info(self, upgrade):
        ...
        values = {key: value for (key, value) in upgrade.items() if key in keys}
        values.update(
            {
                "id": upgrade["api_id"],
                "title": upgrade["title"],
