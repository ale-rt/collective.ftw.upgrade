...
class TestUpgradeInformationGatherer(UpgradeTestCase):
    ...
    def test_not_yet_installed_upgrades_are_marked_as_proposed(self):
        self.package.with_profile(
            ...
            .with_upgrade(Builder("plone upgrade step").upgrading("1001", to="1002"))
        )

        with self.package_created():
            self.install_profile("the.package:default", "1001")
            self.assert_gathered_upgrades(
                {"the.package:default": {"proposed": ["1002"], "done": ["1001"]}}
            )
    ...
    def test_filtering_proposed_upgrades(self):
        ...
        with self.package_created():
            self.install_profile("the.package:default", "1001")
            self.assert_gathered_upgrades(
                {"the.package:default": {"proposed": ["1002"], "done": []}},
                proposed_only=True,
            )
    ...
    def test_profile_with_outdated_fs_version_is_flagged(self):
        self.package.with_profile(
            Builder("genericsetup profile")
            .named("outdated")
            .with_upgrade(Builder("plone upgrade step").upgrading("1111", "2222"))
            .with_fs_version("2222")
            .with_upgrade(Builder("plone upgrade step").upgrading("2222", "3333"))
        )
        ...
        with self.package_created():
            ...
            self.assert_outdated_profiles(
                ["the.package:outdated"],
                ignore=["plone.app.jquerytools:default", "plone.app.jquery:default"],
            )
            self.assert_gathered_upgrades(
                {
                    "the.package:outdated": {
    ...
    def test_profile_only_listed_when_installed(self):
        ...
        self.package.with_profile(Builder("genericsetup profile").named("uninstall"))
    ...
    def test_profiles_are_ordered_by_dependencies(self):
        self.package.with_profile(
            Builder("genericsetup profile")
            .named("one")
            .with_dependencies("the.package:two", "the.package:four")
        ...
        with self.package_created():
            ...
            self.assertEqual(
                ["the.package:four", "the.package:two", "the.package:one"],
                self.get_listed_profiles(),
            )
    ...
    def test_cyclic_dependencies_raises_an_exception(self):
        ...
        with self.package_created():
            ...
            self.assertIn(
                ("the.package:bar", "the.package:foo"), cm.exception.dependencies
            )
    ...
    def test_no_longer_existing_profiles_are_silently_removed(self):
        ...
        with self.package_created():
            self.install_profile("the.package:default")
            self.assertEqual(
                ["the.package:removed", "the.package:default"],
                self.get_listed_profiles(),
            )
            ...
            self.assertEqual(["the.package:default"], self.get_listed_profiles())
    ...
    def test_profile_infos(self):
        self.package.with_profile(
            Builder("genericsetup profile").with_upgrade(self.default_upgrade())
        )
    ...
    def test_human_readable_timestamp_versions_are_added_to_profiles(self):
        self.package.with_profile(
            Builder("genericsetup profile").with_upgrade(
                Builder("ftw upgrade step").to(datetime(2011, 12, 30, 17, 45, 59))
            )
        )
    ...
    def test_get_upgrades_by_api_ids_orders_upgrades(self):
        ...
        with self.package_created():
            ...
            gatherer = queryAdapter(self.portal_setup, IUpgradeInformationGatherer)
            self.assertEqual(
                gatherer.get_upgrades_by_api_ids(
                    "20110101000000@the.package:default",
    ...
    def test_get_upgrades_by_api_ids_raises_upgrade_not_found(self):
        ...
        self.assertEqual(
            'The upgrade "foo@bar:default" could not be found.', str(cm.exception)
        )
    ...
    def get_listed_profiles(self, filter_package="the.package"):
        ...
        if filter_package:
            profiles = [
                profile for profile in profiles if profile.startswith(filter_package)
            ]
        return profiles
...
class TestExtendAutoUpgradesWithHumanFormattedDateVersion(TestCase):
    ...
    def test_formats_timestamp_source_versions(self):
        input = {"ssource": "20141214183059", "sdest": "1"}
        expected = {
            "ssource": "20141214183059",
    ...
    def test_formats_timestamp_dest_versions(self):
        input = {"ssource": "1", "sdest": "20141214183059"}
        expected = {
            "ssource": "1",
    ...
    def test_does_not_format_non_timestamp_versions(self):
        input = {"ssource": "1", "sdest": "2"}
        expected = {"ssource": "1", "sdest": "2"}
        self.assertDictEqual(expected, self.format_upgrade_step(input))

    def test_does_not_fail_on_non_timestamp_versions_looking_like_timestamps(self):
        input = {"ssource": "10000000000000", "sdest": "1"}
        expected = {"ssource": "10000000000000", "sdest": "1"}
        self.assertDictEqual(expected, self.format_upgrade_step(input))

        input = {"ssource": "1", "sdest": "10000000000000"}
        expected = {"ssource": "1", "sdest": "10000000000000"}
        self.assertDictEqual(expected, self.format_upgrade_step(input))
