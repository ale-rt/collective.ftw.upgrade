...
LAYER = TempDirectoryLayer(
    bases=(BUILDER_LAYER,), name="ftw.upgrade.test_command_utils"
)
...
class TestFindEgginfo(TestCase):
    ...
    def test_finds_egginfo_in_current_directory(self):
        ...
        self.assertEqual(
            self.path.joinpath("the.package.egg-info"), find_egginfo(self.path)
        )

    def test_working_directory_used_when_no_path_passed_to_in(self):
        create(self.package_builder)
        with chdir(self.path):
            self.assertEqual(self.path.joinpath("the.package.egg-info"), find_egginfo())
    ...
    def test_prints_warning_when_no_egginfo_found(self):
        ...
        self.assertEqual(
            "WARNING: no *.egg-info directory could be found.\n", stderr.getvalue()
        )
...
class TestFindPackageNamespacePath(TestCase):
    ...
    def test_returns_absolute_path_to_toplevel_namespace_directory(self):
        ...
        self.assertEqual(
            self.path.joinpath("the"), find_package_namespace_path(egginfo)
        )
