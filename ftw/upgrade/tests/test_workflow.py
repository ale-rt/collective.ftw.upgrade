...
class TestWorkflowChainUpdater(WorkflowTestCase):
    ...
    def test_changing_workflow_with_mapping(self):
        self.set_workflow_chain(
            for_type="Folder", to_workflow="intranet_folder_workflow"
        )
        ...
        with WorkflowChainUpdater(objects, mapping):
            self.set_workflow_chain(for_type="Folder", to_workflow="folder_workflow")

        self.assertReviewStates(
            {private_folder: "visible", published_folder: "published"}
        )
    ...
    def test_object_security_is_updated(self):
        ...
        mapping = {("intranet_workflow", "plone_workflow"): {"external": "published"}}
    ...
    def test_object_security_is_reindexed(self):
        ...
        mapping = {("intranet_workflow", "plone_workflow"): {"internal": "published"}}
    ...
    def test_workflow_history_is_not_migrated_when_migration_disabled(self):
        ...
        with WorkflowChainUpdater([folder], mapping, migrate_workflow_history=False):
            self.set_workflow_chain(for_type="Folder", to_workflow="folder_workflow")
    ...
    def test_workflow_history_transition_is_updated_when_mapped(self):
        ...
        with WorkflowChainUpdater(
            [folder], mapping, transition_mapping=transition_mapping
        ):
            self.set_workflow_chain(
                for_type="Folder", to_workflow="intranet_folder_workflow"
            )
    ...
    def assert_workflow_history_entry(self, context, expected, index=-1):
        ...
        self.assertDictContainsSubset(
            expected, history[index], f"Workflow history entry is wrong.\n{history}"
        )
    ...
    def assert_workflow_history_length(self, context, length):
        ...
        history = wftool.getInfoFor(context, "review_history")
        self.assertEqual(
            length, len(history), f"Workflow history length is wrong.\n{history}"
        )
...
class TestWorkflowSecurityUpdater(WorkflowTestCase):
    ...
    def test_updates_only_objects_with_specified_workflows(self):
        ...
        self.set_workflow_chain(
            for_type="Document", to_workflow="simple_publication_workflow"
        )
        ...
        self.assert_permission_not_acquired(
            "View", folder, "The folder should have been updated but it was not."
        )
    ...
    def test_updates_disabling_update_security(self):
        ...
        self.assertEqual(
            ["Reader"], self.get_allowed_roles_and_users(for_object=folder)
        )
        ...
        self.assertEqual(
            ["Anonymous"], self.get_allowed_roles_and_users(for_object=folder)
        )
    ...
    def test_respects_placeful_workflows_when_updating(self):
        ...
        activator.activate_policy(
            ...
            review_state_mapping={
                (None, "simple_publication_workflow"): {None: "private"}
            },
        )
