...
class TestUpgradeStep(UpgradeTestCase):
    ...
    def test_getToolByName(self):
        ...
        class Step(UpgradeStep):
            def __call__(self):
                testcase.assertEqual(actions_tool, self.getToolByName("portal_actions"))
    ...
    def test_objects_method_yields_objects_with_logging(self):
        ...
        self.assertEqual(
            ["STARTING Log message", "1 of 3 (33%): Log message", "DONE Log message"],
            self.get_log(),
        )
    ...
    def test_logs_indexing_progress(self):
        ...
        self.assertEqual(["1 of 3 (33%): Processing indexing queue"], self.get_log())
    ...
    def test_catalog_rebuild_index(self):
        ...
        create(Builder("folder").titled("Rebuild Index Test Obj"))
    ...
    def test_catalog_reindex_objects_keeps_modification_date(self):
        ...
        class Step(UpgradeStep):
            def __call__(self):
                ...
                brain = self.catalog_unrestricted_search({"UID": folder.UID()})[0]
    ...
    def test_catalog_unrestricted_get_object(self):
        ...
        class Step(UpgradeStep):
            def __call__(self):
                query = {"path": folder_path, "portal_type": "Document"}
                brains = self.catalog_unrestricted_search(query)

                brain = brains[0]
                testcase.assertEqual("ImplicitAcquisitionWrapper", type(brain).__name__)

                obj = self.catalog_unrestricted_get_object(brain)
                testcase.assertEqual("ImplicitAcquisitionWrapper", type(obj).__name__)
    ...
    def test_catalog_unrestricted_get_object_removes_dead_brains(self):
        ...
        aq_parent(aq_inner(folder))._delObject(folder.getId(), suppress_events=True)
    ...
    def test_catalog_unrestricted_search(self):
        ...
        class Step(UpgradeStep):
            def __call__(self):
                ...
                testcase.assertEqual(
                    {"page-one", "page-two"}, {brain.id for brain in brains}
                )
                testcase.assertEqual(
                    "ImplicitAcquisitionWrapper", type(brains[0]).__name__
                )
                ...
                testcase.assertEqual(
                    {"page-one", "page-two"}, {obj.id for obj in objects}
                )
                testcase.assertEqual(
                    "ImplicitAcquisitionWrapper", type(objects[0]).__name__
                )
    ...
    def test_catalog_unrestricted_search_filters_nonexisting_objects(self):
        ...
        class Step(UpgradeStep):
            def __call__(self):
                ...
                testcase.assertEqual(1, len(self.catalog_unrestricted_search(query)))
                ...
                testcase.assertEqual((), tuple(self.catalog_unrestricted_search(query)))
    ...
    def test_actions_remove_action(self):
        ...
        class Step(UpgradeStep):
            def __call__(self):
                testcase.assertTrue(
                    self.actions_remove_action("document_actions", "rss")
                )
    ...
    def test_actions_remove_type_action(self):
        ...
        class Step(UpgradeStep):
            ...
            def __call__(self):
                ...
                testcase.assertTrue(self.actions_remove_type_action("Event", "view"))
    ...
    def test_actions_add_type_action(self):
        ...
        class Step(UpgradeStep):
            ...
            def __call__(self):
                testcase.assertEqual(None, self.get_event_action("additional"))
                testcase.assertNotIn("additional", self.get_action_ids())
                ...
                testcase.assertIn("additional", self.get_action_ids())
    ...
    def test_add_lines_to_property(self):
        ...
        class Step(UpgradeStep):
            def __call__(self):
                ...
                testcase.assertEqual(("foo",), self.portal.getProperty(key))

                self.add_lines_to_property(self.portal, key, ["bar"])
                testcase.assertEqual(("foo", "bar"), self.portal.getProperty(key))

                self.add_lines_to_property(self.portal, key, "baz")
                testcase.assertEqual(
                    ("foo", "bar", "baz"), self.portal.getProperty(key)
                )
    ...
    def test_setup_install_profile(self):
        self.package.with_profile(
            Builder("genericsetup profile").with_file(
                "catalog.xml", self.asset("exclude-from-nav-index.xml")
            )
        )
    ...
    @skipIf(
        pkg_resources.get_distribution("Products.GenericSetup").parsed_version
    ...
    def test_ensure_profile_installed(self):
        self.package.with_profile(
            Builder("genericsetup profile").with_fs_version("1111")
        )
    ...
    def test_install_upgrade_profile(self):
        class AddSiteProperty(UpgradeStep):
            def __call__(self):
                assert not self.portal.getProperty(
                    "foo"
                ), 'property "foo" should not yet exist'
                self.install_upgrade_profile()
                assert (
                    self.portal.getProperty("foo") == "bar"
    ...
    def test_is_product_installed(self):
        ...
        class Step(UpgradeStep):
            def __call__(self):
                testcase.assertTrue(self.is_product_installed("CMFPlacefulWorkflow"))
                self.uninstall_product("CMFPlacefulWorkflow")
                testcase.assertFalse(self.is_product_installed("CMFPlacefulWorkflow"))

        Step(self.portal_setup)

    def test_is_profile_installed(self):
        self.package.with_profile(
            Builder("genericsetup profile").with_fs_version("1000")
        )
        testcase = self
        ...
        class Step(UpgradeStep):
            def __call__(self):
                testcase.assertFalse(
                    self.is_profile_installed("profile-the.package:default")
                )
                testcase.assertFalse(self.is_profile_installed("the.package:default"))
                self.setup_install_profile("profile-the.package:default")
                testcase.assertTrue(
                    self.is_profile_installed("profile-the.package:default")
                )
                testcase.assertTrue(self.is_profile_installed("the.package:default"))
    ...
    def test_migrate_class(self):
        ...
        self.assertIn(subfolder.__class__.__name__, ("ATFolder", "Folder"))
        Step(self.portal_setup)
    ...
    def test_migrate_class_also_updates_provided_interfaces_info(self):
        ...
        Step(self.portal_setup)
        self.assertFalse(
            IDocument.providedBy(obj), "Document interface not removed in migration"
        )
        self.assertTrue(ILink.providedBy(obj), "Link interface not added in migration")
    ...
    def test_remove_broken_browserlayer(self):
        ...
        class Step(UpgradeStep):
            def __call__(self):
                self.remove_broken_browserlayer("my.product", "IMyProductLayer")
        ...
        self.assertNotIn(
            (ILocalBrowserLayerType, "my.product"), sm._utility_registrations
        )

    def test_remove_broken_browserlayer_doesnt_fail_if_layer_missing(self):
        class Step(UpgradeStep):
            def __call__(self):
                self.remove_broken_browserlayer(
                    "my.nonexistent.product", "IMyNonexistentProductLayer"
                )
    ...
    def test_remove_remove_broken_portlet_manager(self):
        ...
        sm = self.portal.getSiteManager()
        sm.registerUtility(
            component=PortletManager(), provided=IPortletManager, name="my.manager"
        )
        ...
        self.assertIsNotNone(sm.queryUtility(IPortletManager, name="my.manager"))
    ...
    def test_update_security_removes_roles_unmanaged_by_workflow(self):
        self.set_workflow_chain(for_type="Folder", to_workflow="plone_workflow")
        folder = create(Builder("folder"))
        ...
        folder.manage_permission("Modify portal content", roles=[], acquire=True)
        folder.manage_permission("List folder contents", roles=[], acquire=False)
    ...
    def test_update_security_reindexes_security_indexes(self):
        self.set_workflow_chain(
            for_type="Folder", to_workflow="simple_publication_workflow"
        )
        folder = create(Builder("folder").in_state("published"))

        self.assertEqual(["Anonymous"], self.get_allowed_roles_and_users_for(folder))
        ...
        self.assertEqual(["Reader"], self.get_allowed_roles_and_users_for(folder))
    ...
    def test_update_workflow_security_reindexes_security(self):
        ...
        class Step(UpgradeStep):
            def __call__(self):
                self.update_workflow_security(["plone_workflow"], reindex_security=True)
    ...
    def test_update_workflow_security_expects_list_of_workflows(self):
        ...
        self.assertEqual(
            '"workflows" must be a list of workflow names.', str(cm.exception)
        )
    ...
    def test_base_profile_and_target_version_are_stored_in_attribute(self):
        ...
        self.assertEqual(
            {"base_profile": "profile-ftw.upgrade:default", "target_version": 1500},
            result,
        )
