...
class TestPloneSiteJsonApi(JsonApiTestCase):
    ...
    def test_get_profile(self, browser):
        ...
        with self.package_created():
            ...
            self.assertEqual(
                "application/json; charset=utf-8", browser.headers.get("content-type")
            )
    ...
    @browsing
    def test_get_unknown_profile_returns_error(self, browser):
        with self.expect_api_error(
            status=400,
    ...
    def test_execute_proposed_upgrades(self, browser):
        ...
        with self.package_created():
            ...
            self.assertIn(
                "UPGRADE STEP the.package:default: The upgrade.", browser.contents
            )
            self.assertIn("Result: SUCCESS", browser.contents)
    ...
    def test_execute_profiles_standard(self, browser):
        ...
        with self.package_created():
            ...
            self.assertIn(
                "Done installing profile the.package:default.", browser.contents
            )
            self.assertIn("Result: SUCCESS", browser.contents)
