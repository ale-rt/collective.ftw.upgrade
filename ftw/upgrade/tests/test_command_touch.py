...
class TestTouchCommand(CommandTestCase):
    ...
    def test_touching_an_upgrade_step_renews_the_timestamp(self):
        ...
        self.assertFalse(path.exists(), f"Expected path to no longer exist: {path}")
    ...
    def test_moving_explicitly_to_the_end(self):
        ...
        self.upgrade_script(
            "touch {remove_action} --after {add_action}".format(**self.upgrades())
        )
    ...
    def test_moving_before_another(self):
        ...
        self.upgrade_script(
            "touch {update_action} --before {remove_action}".format(**self.upgrades())
        )
    ...
    def test_moving_explicitly_to_the_beginning(self):
        ...
        self.upgrade_script(
            "touch {add_action} --before {remove_action}".format(**self.upgrades())
        )
    ...
    def test_moving_before_and_after_at_same_time_is_not_allowed(self):
        ...
        self.assertEqual(
            2,
            exitcode,
            "command should fail because --after and --before can"
            " not be used at the same time.",
        )
        self.assertIn(
            "error: argument --after/-a: not allowed with argument --before/-b", output
        )
    ...
    def upgrades(self):
        ...
        self.assertTrue(
            upgrades_dir.isdir(), f'"upgrades" directory is missing at {upgrades_dir}'
        )
