...
class TestTopologicalSort(TestCase):
    ...
    def test_simple(self):
        ...
        self.assertEqual(["a", "b", "c"], topological_sort(items, dependencies))

    def test_advanced(self):
        items = ["a", "c", "b", "d"]
        dependencies = (("a", "b"), ("a", "c"), ("b", "d"), ("b", "c"))

        self.assertEqual(["a", "b", "c", "d"], topological_sort(items, dependencies))

    def test_duplicated(self):
        items = ["a", "b", "a"]
        dependencies = (("b", "a"),)

        self.assertEqual(["b", "a"], topological_sort(items, dependencies))

    def test_cyclic(self):
        items = ["a", "b"]
        dependencies = (("a", "b"), ("b", "a"))

        self.assertEqual(None, topological_sort(items, dependencies))

    def test_root_nodes_are_reversed_ordered(self):
        items = ["policy", "xy", "foo"]

        self.assertEqual(
            ["xy", "policy", "foo"], topological_sort(items, (("policy", "foo"),))
        )
...
class TestFindCyclicDependencies(TestCase):
    ...
    def test_direct_cyclic_dependencies(self):
        ...
        self.assertEqual(
            [{"foo", "bar"}], list(map(set, find_cyclic_dependencies(dependencies)))
        )
...
class TestSortedProfileIds(MockTestCase):
    ...
    def test_dependencies_resolved(self):
        ...
        listProfileInfo.return_value = [
            {"id": "baz", "dependencies": ["profile-foo", "profile-bar"]},
            {"id": "foo"},
            {"id": "bar", "dependencies": ["profile-foo"]},
        ]
        portal_setup = self.create_dummy()
        portal_setup.listProfileInfo = listProfileInfo

        self.assertEqual(["foo", "bar", "baz"], get_sorted_profile_ids(portal_setup))
    ...
    def test_root_profiles_are_ordered_by_profile_name(self):
        ...
        listProfileInfo = self.mock()
        listProfileInfo.return_value = [
            {"id": "baz", "dependencies": ["profile-foo"]},
            {"id": "foo"},
            {"id": "xy"},
        ]
        portal_setup = self.create_dummy()
        ...
        self.assertEqual(["foo", "baz", "xy"], get_sorted_profile_ids(portal_setup))

    def test_cyclic_dependencies(self):
        listProfileInfo = self.mock()
        listProfileInfo.return_value = [
            {"id": "foo", "dependencies": ["profile-bar"]},
            {"id": "bar", "dependencies": ["profile-foo"]},
            {"id": "baz", "dependencies": []},
        ]
        portal_setup = self.create_dummy()
        ...
        self.assertEqual([("foo", "bar"), ("bar", "foo")], cm.exception.dependencies)
...
class TestFormatDuration(TestCase):
    ...
    def test_single_minute_is_singular(self):
        self.assertEqual(
            ["1 minute", "1 minute, 1 second", "1 minute, 2 seconds"],
            [format_duration(60), format_duration(60 + 1), format_duration(60 + 2)],
        )

    def test_multiple_minutes_is_plural(self):
        self.assertEqual(
            ["2 minutes, 1 second", "2 minutes, 2 seconds"],
            [format_duration((2 * 60) + 1), format_duration((2 * 60) + 2)],
        )
    ...
    def test_floating_point_seconds_are_ceiled(self):
        self.assertEqual(
            ["1 second", "1 second", "2 seconds", "2 seconds"],
            [
                format_duration(0.1),
                format_duration(0.9),
                format_duration(1.1),
...
class TestSubjectFromDocstring(TestCase):
    ...
    def test_one_line_only(self):
        self.assertEqual(
            "This is the subject.", subject_from_docstring("This is the subject.")
        )

    def test_whitespace_is_stripped(self):
        self.assertEqual(
            "This is the subject.", subject_from_docstring("\nThis is the subject.\n")
        )
...
class TestMemoryControl(TestCase):
    ...
    def test_memory_is_full(self):
        load = {"memory_available": 1000 * 1024 * 1024, "memory_percent": 10}
        self.assertFalse(_is_memory_full(load, LOAD_LIMITS))

        load = {"memory_available": 0, "memory_percent": 10}
        self.assertTrue(_is_memory_full(load, LOAD_LIMITS))

        load = {"memory_available": 1000 * 1024 * 1024, "memory_percent": 99}
        self.assertTrue(_is_memory_full(load, LOAD_LIMITS))

    def test_is_memory_critical(self):
        load_limits = {"memory_available": 0, "memory_percent": 100}
        self.assertFalse(is_memory_critical(load_limits))

        load_limits = {"memory_available": 0, "memory_percent": 0}
