...
class TestDirectoryMetaDirective(UpgradeTestCase):
    ...
    def test_upgrade_steps_are_registered(self):
        self.profile.with_upgrade(
            Builder("ftw upgrade step").to(datetime(2011, 1, 1, 8)).named("add_action")
        )
        self.profile.with_upgrade(
    ...
    def test_profile_must_be_registed_before_registering_upgrade_directory(self):
        package_builder = (
            ...
            .with_zcml_include("ftw.upgrade", file="meta.zcml")
            .with_zcml_node(
                "upgrade-step:directory", profile="other.package:default", directory="."
            )
        )
    ...
    def test_profile_version_is_set_to_latest_old_school_profile_version(self):
        ...
        profile_path = package.package_path.joinpath("profiles", "default")
        self.assertNotIn("<version", profile_path.joinpath("metadata.xml").text())
    ...
    def test_declaring_upgrades_dependency(self):
        self.package.with_profile(
            ...
            .with_upgrade(
                ...
                .with_zcml_directory_options(soft_dependencies="the.package:baz")
            )
        )
        ...
        self.package.with_profile(
            ...
            .with_upgrade(
                Builder("ftw upgrade step")
                .to(datetime(2010, 1, 1, 1, 1))
                .with_zcml_directory_options(soft_dependencies="the.package:default")
            )
        )
            ...
            self.assert_profile(
                {
                    ...
                    "title": "the.package",
                    "description": "",
                    "ftw.upgrade:dependencies": ["the.package:bar", "the.package:baz"],
    ...
    def test_handler_step_provides_interfaces_implemented_by_upgrade_step_class(self):
        ...
        self.profile.with_upgrade(
            Builder("ftw upgrade step").to(datetime(2011, 1, 1, 1)).with_code(code)
        )

        with self.package_created():
            portal_setup = getToolByName(self.portal, "portal_setup")
            steps = listUpgradeSteps(
                portal_setup, "the.package:default", "10000000000000"
            )
            self.assertEqual(1, len(steps))
            self.assertCountEqual(
                (IRecordableHandler, IUpgradeStep, IFoo),
