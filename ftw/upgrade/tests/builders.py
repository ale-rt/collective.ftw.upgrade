...
class UpgradeStepBuilder:
    ...
    def create(self):
        if self.destination_version is None:
            raise ValueError(
                "A destination version is required." " Use .to(datetime(...))."
            )
        self._set_package()
    ...
    def _create_upgrade(self):
        ...
        self.package.with_file(
            os.path.join(step_name, "__init__.py"), "", makedirs=True
        )

        self.package.with_file(os.path.join(step_name, "upgrade.py"), self.code)
        return step_name
